...
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'pilot-portal-dev-key-change-in-production')
...
def secure_file_upload(file, pilot_id):
    ...
    try:
        ...
        # Validate file content type using python-magic (if available)
        try:
        ...
        secure_name = f"{timestamp}_{pilot_id}_{file_hash}.{ext}"
        
        # Save file
        filepath = os.path.join(UPLOAD_FOLDER, secure_name)
        file.save(filepath)
        
        logger.info(f"File uploaded successfully: {secure_name}")
        return secure_name
...
# Initialize scheduler with proper shutdown
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        # Check if work_email column exists, add it if not
        ...
        # Flight entries table - FIXED with proper column structure
        cursor.execute('''
        ...
        # Check if photo_filename column exists, add it if not (for existing databases)
        ...
        if 'photo_filename' not in columns:
            ...
            cursor.execute('UPDATE flight_entries SET photo_filename = "no_photo.jpg" WHERE photo_filename IS NULL')
            conn.commit()
...
class ExcelGenerator:
    def generate_monthly_excel(self, pilot_id, year, month):
        ...
        # Get month name and aircraft list
...
class ImprovedEmailSender:
    """Enhanced email sender with immediate ops notification, auto-cleanup, and admin alerts"""
    ...
    def _validate_config(self) -> None:
        ...
        if not self.gmail_user or not self.gmail_password:
            raise ValueError("Gmail credentials not configured. Check GMAIL_USER and GMAIL_PASSWORD environment variables.")
        
        if len(self.gmail_password) < 16:
            logger.warning("Gmail password seems short - ensure you're using an App Password")
        
        if not self.admin_email:
            logger.warning("ADMIN_EMAIL not configured - admin notifications disabled")
    
    @contextmanager
    def _get_smtp_connection(self):
        """Context manager for SMTP connections with proper cleanup"""
        server = None
        try:
    ...
    def _create_message(self, to_email: str, subject: str, body: str, 
        ...
        if attachment_path and os.path.exists(attachment_path):
            try:
                ...
                msg.attach(part)
                logger.debug(f"Attachment added: {filename} ({len(file_data)} bytes)")
                attachment_status = f"Attachment included: {filename}"
                
            except Exception as e:
                logger.error(f"Failed to attach file {attachment_path}: {e}")
                attachment_status = f"ERROR: Could not attach {os.path.basename(attachment_path)}"
        
        elif attachment_path and not os.path.exists(attachment_path):
            logger.warning(f"Attachment file not found: {attachment_path}")
            attachment_status = f"ERROR: File not found - {os.path.basename(attachment_path)}"
        
        # Add attachment status to body if there were issues
        if "ERROR" in attachment_status:
            error_note = f"\n\n--- SYSTEM NOTICE ---\n{attachment_status}\nPlease contact pilot for logsheet image."
            msg.attach(MIMEText(error_note, 'plain'))
        
        return msg
    
    def _send_batch_emails(self, recipients: List[str], subject: str, body: str, 
                          attachment_path: Optional[str] = None) -> Tuple[int, int, List[str]]:
        """
        Send emails to multiple recipients efficiently
        Returns: (success_count, total_count, failed_recipients)
        """
        if not recipients:
            return 0, 0, []
            ...
            try:
                with self._get_smtp_connection() as server:
                    for recipient in recipients:
                        ...
                        try:
                            message = self._create_message(recipient, subject, body, attachment_path)
                            server.sendmail(self.gmail_user, recipient, message.as_string())
                ...
                logger.error(f"Batch email attempt {attempt + 1} failed: {e}")
        ...
        logger.info(f"Batch email complete: {success_count}/{total_count} sent successfully")
        return success_count, total_count, failed_recipients
    
    def send_logsheet_to_ops(self, flight_entry_id: int) -> bool:
        """Send logsheet to operations immediately with failure notifications"""
        ...
        try:
            # Get active ops emails - REQUIREMENT: At least 1 address
            cursor.execute("SELECT email FROM ops_emails WHERE is_active = TRUE")
            ops_emails = [row[0] for row in cursor.fetchall()]
            
            if not ops_emails:
                logger.error("CRITICAL: No active ops emails configured!")
                self._send_admin_alert("CRITICAL: No Ops Emails Configured", 
                    "A logsheet was submitted but no active operations emails are configured. "
                    "Logsheet was NOT delivered to operations!")
                conn.close()
                return False
            ...
            # Prepare email - REQUIREMENT: Proper subject line and attachment
            photo_filename = entry[11]
            attachment_path = os.path.join('uploads', photo_filename) if photo_filename else None
            
            # Subject format: "Frosty's Logsheet ##### Q##### date"
            subject = f"Frosty's Logsheet {entry[4]} Q{entry[5]} {entry[6]}"
            
            # Minimal body - attachment is the key requirement
...
"""
            ...
            # Send to ops emails immediately
            success_count, total_count, failed_recipients = self._send_batch_emails(
                recipients=ops_emails,
                subject=subject,
                body=body,
                attachment_path=attachment_path
            )
            
            # Update database if ALL emails sent successfully
            if success_count == total_count and total_count > 0:
                ...
                conn.commit()
                logger.info(f"✅ Logsheet {entry[4]} sent to {success_count} operations emails")
                conn.close()
                return True
            else:
                # REQUIREMENT: Admin notification for failures
                if failed_recipients:
                    self._send_admin_alert(
                        f"Logsheet Email Delivery Failed - {entry[4]}",
                        f"Logsheet {entry[4]} from {entry[13]} could not be delivered to:\n"
                        f"Failed: {', '.join(failed_recipients)}\n"
                        f"Successful: {success_count}/{total_count}\n"
                        f"Please check email configuration and resend manually."
                    )
                
                logger.warning(f"⚠️ Partial email success for logsheet {entry[4]}: {success_count}/{total_count}")
                conn.close()
            ...
            self._send_admin_alert(
                "Logsheet Email System Error",
                f"Critical error sending logsheet {flight_entry_id}:\n{str(e)}\n"
                f"Please check system logs and resend manually."
            )
            conn.rollback()
            conn.close()
            return False
    
    def send_monthly_reports(self, year: int, month: int) -> bool:
        """Send monthly reports and cleanup old photos - REQUIREMENT: Delete photos after reports sent"""
        from app import db_manager, ExcelGenerator
        
        conn = db_manager.get_connection()
        cursor = conn.cursor()
        
        # Get pilots with flights for this month
        month_start = datetime(year, month, 1).date()
        if month == 12:
            month_end = datetime(year + 1, 1, 1).date() - timedelta(days=1)
        else:
        ...
        if not pilots_with_flights:
            logger.info("No pilots with flights found for monthly reports")
            conn.close()
            return True
        ...
        # Get photos from this month for cleanup
        cursor.execute('''
            SELECT DISTINCT photo_filename
            FROM flight_entries 
            WHERE flight_date >= ? AND flight_date <= ? 
            AND photo_filename IS NOT NULL 
            AND photo_filename != 'legacy_no_photo.jpg'
        ''', (month_start, month_end))
        
        photos_to_cleanup = [row[0] for row in cursor.fetchall()]
        ...
        for pilot_id, pilot_name, personal_email, work_email in pilots_with_flights:
            try:
                ...
                if not filepath:
                    logger.warning(f"No Excel report generated for {pilot_name}")
                    continue
                
                generated_files.append(filepath)
                
                # Use work email if available, otherwise personal
                send_to_email = work_email if work_email else personal_email
                if not send_to_email:
                    logger.error(f"No email address found for pilot {pilot_name}")
                    continue
...
This report includes all your flight entries for the month with details on:
- Flight dates and aircraft
- Logsheet and Q numbers  
- Flight times and types
- Applied rates and earnings

If you have any questions about your report, please contact Frosty's operations.

Best regards,
Frosty's Operations Team
Professional Flight Services
"""
                
                success_count_single, total_count_single, failed = self._send_batch_emails(
                ...
                if success_count_single == total_count_single:
                    success_count += 1
                    logger.info(f"✅ Monthly report sent to {pilot_name} at {send_to_email}")
                else:
        ...
        # REQUIREMENT: Cleanup photos only AFTER successful report sending
        if success_count == len(pilots_with_flights):
            logger.info(f"All reports sent successfully. Cleaning up {len(photos_to_cleanup)} photos...")
            
            for photo_filename in photos_to_cleanup:
                try:
                    photo_path = os.path.join('uploads', photo_filename)
                    if os.path.exists(photo_path):
                        os.remove(photo_path)
                        logger.debug(f"Cleaned up photo: {photo_filename}")
                except Exception as e:
                    logger.warning(f"Could not clean up photo {photo_filename}: {e}")
        else:
            logger.warning(f"Not all reports sent successfully ({success_count}/{len(pilots_with_flights)}). Keeping photos.")
        
        # Always cleanup generated Excel files
        for filepath in generated_files:
            try:
                if os.path.exists(filepath):
                    os.remove(filepath)
                    logger.debug(f"Cleaned up Excel file: {filepath}")
            except Exception as e:
                logger.warning(f"Could not clean up Excel file {filepath}: {e}")
        ...
        if success_count > 0:
            self._send_admin_alert(
                f"Monthly Reports Sent - {month_start.strftime('%B %Y')}",
                f"Successfully sent {success_count}/{len(pilots_with_flights)} monthly reports.\n"
                f"Cleaned up {len(photos_to_cleanup)} photo files."
            )
        
        logger.info(f"📊 Monthly reports: {success_count}/{len(pilots_with_flights)} sent successfully")
        return success_count > 0
    
    def _send_admin_alert(self, subject: str, message: str) -> None:
        """REQUIREMENT: Send immediate admin notifications for failures"""
        if not self.admin_email:
            logger.warning(f"Admin alert not sent (no admin email): {subject}")
            return
        
        try:
            alert_subject = f"🚨 Frosty's Alert: {subject}"
            alert_body = f"""FROSTY'S PILOT PORTAL ALERT
...
Please check the admin dashboard and take corrective action.
...
"""
            ...
            success, total, failed = self._send_batch_emails([self.admin_email], alert_subject, alert_body)
            if success == total:
                logger.info(f"✅ Admin alert sent: {subject}")
            else:
                logger.error(f"❌ Failed to send admin alert: {subject}")
    ...
    def test_email_configuration(self) -> Dict[str, bool]:
        ...
        results = {
            'credentials_configured': bool(self.gmail_user and self.gmail_password),
            'smtp_connection': False,
            'email_send_test': False,
            'ops_emails_configured': False
        }
        ...
        # Check ops emails configuration
        try:
            from app import db_manager
            conn = db_manager.get_connection()
            cursor = conn.cursor()
            ...
            results['ops_emails_configured'] = ops_count > 0
            conn.close()
        except Exception as e:
            logger.error(f"Ops email check failed: {e}")
        
        # Test actual email sending
        if self.admin_email and results['smtp_connection']:
            try:
                test_subject = "✅ Frosty's Email System Test - SUCCESS"
                test_body = f"""Email system test completed successfully!

Configuration Status:
- Gmail Connection: ✅ Working
- SMTP Server: {self.smtp_server}:{self.smtp_port}
- From Address: {self.gmail_user}
- Operations Emails: {'✅ Configured' if results['ops_emails_configured'] else '❌ Not configured'}

Test completed at: {datetime.now()}

The email system is ready for logsheet submissions.
...
"""
                ...
                success, total, failed = self._send_batch_emails([self.admin_email], test_subject, test_body)
                results['email_send_test'] = success > 0
                
            except Exception as e:
                logger.error(f"Email send test failed: {e}")
...
def security_headers():
    ...
    # Simple session check - no timeout for now to avoid datetime issues
...
def pilot_auth():
    ...
    if pilot:
        ...
        session['pilot_name'] = pilot[1]
        session['is_admin'] = pilot[2]
        
        if pilot[2]:  # Admin user
...
def pilot_dashboard():
    ...
    cursor.execute('''
        SELECT fe.flight_date, a.registration, fe.logsheet_number, fe.q_number,
               fe.flight_type, fe.airtime, fe.amount_earned
        FROM flight_entries fe
        JOIN aircraft a ON fe.aircraft_id = a.id
        WHERE fe.pilot_id = ?
...
def submit_logsheet():
    ...
    # Clear any existing logsheet session data
...
def logsheet_step(step):
    ...
    if request.method == 'POST':
        ...
        if step == 'aircraft':
            ...
            logsheet_data['logsheet_number'] = request.form.get('logsheet_number')
        elif step == 'q_number':
            logsheet_data['q_number'] = request.form.get('q_number')
    ...
    if step == 'aircraft':
        ...
        # Get other pilots assigned to same aircraft
...
def logsheet_submit():
    ...
    try:
        ...
        if email_success:
            flash('Logsheet submitted successfully and sent to ops!', 'success')
        else:
...
def pilot_earnings():
    """Separate earnings page for pilots"""
    if 'pilot_id' not in session:
        return redirect(url_for('pilot_login'))
...
def admin_delete_pilot(pilot_id):
    ...
    # Get pilot name for confirmation message
...
def admin_delete_aircraft(aircraft_id):
    ...
    # Get aircraft registration for confirmation message
...
def admin_delete_ops_email(email_id):
    ...
    # Get email address for confirmation message
